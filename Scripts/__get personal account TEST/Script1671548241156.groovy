...
String verifyKyc = Custom.isServiceCall("$verifyKycLevel")
println("verifyKyc : "+ verifyKyc)



String body = """{'account_number': ${Custom.isServiceCall("$account_number")} }"""


println("body : "+body)

ResponseObject response = Custom.sendRequest('/v1/Account/GetPersonalAccount', body, "POST",
	"$process_description", "$expected_status", "$expected_code", "$expected_message",
	"$expected_ResponseStatusCode", "GetPersonalAccount - TEST",  true )
